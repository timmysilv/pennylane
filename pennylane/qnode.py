...
class QNodeCluster:
    ...
    def __init__(
            ...
            devices: Union[Device, Sequence[Device]],
            interface: str = "autograd",
            diff_method: Union[str, Callable, Sequence[Union[str, Callable]]] = "best",
        ...
        self._funcs = funcs if isinstance(funcs, Sequence) else [funcs]
        self._devices = devices if isinstance(devices, Sequence) else [devices]

        for device in self._devices:
            if not isinstance(device, Device):
                raise qml.QuantumFunctionError(
                    f"Invalid device {device}. Device must be a valid PennyLane device."
                )

        for func in self._funcs:
            if "shots" in inspect.signature(func).parameters:
                raise ValueError(
                    f"Detected 'shots' as an argument to the quantum function {func}. "
