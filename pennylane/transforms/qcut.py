        ...
        for fragment in fragment_tapes:
            with QuantumTape() as tape_config:
                ...
                for meas in fragment.measurements:
                    if meas.obs is None:
                        qml.sample(qml.Projector([1], wires=meas.wires))
                    else:
                        apply(meas)
...
def process_samples(fragment_tapes, configurations, dev):
    """
    The results of executing the tapes generated by the radom configurations
    will contain samples from the Pauli basis at the MeasureNode locations.
    In the absence of MC sampling these samples should not contribute to the
    overall result so are removed here.

    Args:
        fragment_tapes: The result of calling `qcut.sample_graph_to_tape()` over
            each subgraph. These tapes contain the MeasureNode operations.
        configurations: The random configurations of fragment tapes.
        dev: device to execute the random configurations of fragment tapes on.
    """

    mn_idxs = []
    for i, frag_tape in enumerate(fragment_tapes):
        op_names = [op.name for op in frag_tape.operations]
        mn_idxs.append((i, op_names.count("MeasureNode")))

    # This specific way of executing fragments is the initial attempt at
    # ordering rsults so that measurenode samples can be removed. This may
    # need to be updated if we want to use a decorator for sampled cirucit cutting
    res = []
    for frags in configurations:
        frag_res = []
        for frag in frags:
            frag_res.append(frag.execute(dev))
        res.append(frag_res)

    updated_res = []
    copy_res = copy.deepcopy(res)
    for config_samples in copy_res:
        for idxs in mn_idxs:
            frag_num, measure_node_num = idxs
            updated_config_wire = np.delete(
                config_samples[frag_num], list(range(measure_node_num)), axis=0
            )
            config_samples[frag_num] = updated_config_wire
        updated_res.append(config_samples)

    return [np.concatenate(config_results).T for config_results in updated_res]
